...
export function TopGlow({ height, opacity }: { height: number; opacity?: Animated.AnimatedInterpolation<number> }) {
  ...
  return (
    <Animated.View className="absolute top-0 right-0 left-0" style={{ height, opacity: opacity ?? 1 }} pointerEvents="none">
      <LinearGradient
        colors={
          isDark
...
export function BottomGlow() {
  ...
  return (
    <View className="absolute right-0 bottom-0 left-0 h-32" pointerEvents="none">
      <LinearGradient
        colors={
          isDark
